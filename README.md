...
الحسابان المصرّح لهما مضبوطان في `ADMIN_EMAILS`.

كلمة المرور تُقرأ من أول مصدر متاح:

1. `data/auth.json` — عند تغييرها من اللوحة أو بـ `npm run set-password` (مستثنى من git)
2. `ADMIN_PASSWORD_HASH` — متغيّر بيئة، وهو الأنسب عند الاستضافة
3. `src/lib/auth/default-password.ts` — بصمة مدمجة في الكود، فيعمل أي نسخ جديد فورًا

**المخزَّن دائمًا بصمة `scrypt` أحادية الاتجاه، لا كلمة المرور.** وجودها في مستودع
عام آمن ما دامت كلمة المرور قوية وعشوائية (الحالية ٢٠ محرفًا ≈ ١١٦ بت) — كسرها
دون اتصال ضد `scrypt` غير عملي. لكن لو غيّرتها إلى كلمة ضعيفة، تصبح البصمة
المنشورة قابلة للتخمين؛ عندها استخدم `ADMIN_PASSWORD_HASH` بدلًا من الكود.
...
يطبع البرنامج البصمة لتضعها في `ADMIN_PASSWORD_HASH`، ويكتبها في `data/auth.json`
محليًا. **كلمة المرور نفسها لا تُكتب في أي ملف.**
